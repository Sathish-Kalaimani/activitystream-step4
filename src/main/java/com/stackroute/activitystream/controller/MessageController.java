...
public class MessageController {
	...
	*/
	...
	public ResponseEntity<Message> sendMessageToCircle(@PathVariable("circleName") String circleName,@RequestBody Message message, HttpSession session){
		...
		boolean isSent = messageDAO.sendMessageToCircle(circleName, message);
		if(isSent) {
	...
	*/
	...
	public ResponseEntity<Message> sendMessageToUser(@PathVariable("receiverId") String receiverId,@RequestBody Message message, HttpSession session){
		...
		message.setSenderName(loggedInUserName);
		boolean isSent = messageDAO.sendMessageToUser(receiverId, message);
		if(!isSent) {
			return new ResponseEntity<Message>(HttpStatus.INTERNAL_SERVER_ERROR);
		}else {
			return new ResponseEntity<Message>(HttpStatus.OK);
	...
	*/
	...
	public ResponseEntity<List<Message>> getMessageByUser(@PathVariable("senderUsername") String senderUsername, @PathVariable("receiverUserName") String receiverUsername, @PathVariable("pageNumber") int pageNumber, HttpSession session){
		String loggedInUserName = (String) session.getAttribute("loggedInUserName");
		if(loggedInUserName == null) {
			return new ResponseEntity<List<Message>>(HttpStatus.UNAUTHORIZED);
		}else {
			return new ResponseEntity<List<Message>>(messageDAO.getMessagesFromUser(senderUsername, receiverUsername, pageNumber),HttpStatus.OK);
		}
	}
	...
	*/
	...
	public ResponseEntity<List<Message>> getMessagesByCircle(@PathVariable("circleName") String circleName, @PathVariable("pageNumber") int pageNumber, HttpSession session){
		...
		return new ResponseEntity<List<Message>>(messageDAO.getMessagesFromCircle(circleName, pageNumber),HttpStatus.OK);
	}
	...
	*/
	...
	public ResponseEntity<UserTag> subscribeTag(@PathVariable("username") String username, @PathVariable("tag") String tag, HttpSession session){
		String loggedInUserName = (String) session.getAttribute("loggedInUserName");
		if(loggedInUserName == null) {
			return new ResponseEntity<UserTag>(HttpStatus.UNAUTHORIZED);
		}
		boolean isSubscribed = messageDAO.subscribeUserToTag(username, tag);
		if(isSubscribed) {
	...
	*/
	...
	public ResponseEntity<UserTag> unsubscribeTag(@PathVariable("username") String username, @PathVariable("tag") String tag, HttpSession session){
		String loggedInUserName = (String) session.getAttribute("loggedInUserName");
		if(loggedInUserName == null) {
			return new ResponseEntity<UserTag>(HttpStatus.UNAUTHORIZED);
		}
		boolean isunSubscribed = messageDAO.unsubscribeUserToTag(username, tag);
		if(isunSubscribed) {
			return new ResponseEntity<UserTag>(HttpStatus.OK);
		}else {
			return new ResponseEntity<UserTag>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	*/
	...
	public ResponseEntity<List<String>> searchUserTag(@PathVariable("username")String username, HttpSession session){
		String loggedInUserName = (String) session.getAttribute("loggedInUserName");
		if(loggedInUserName ==null) {
			return new ResponseEntity<List<String>>(HttpStatus.UNAUTHORIZED);
		}else {
